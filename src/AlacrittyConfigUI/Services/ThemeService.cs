...
using System.Text;
using AlacrittyConfigUI.Models;
using Serilog;

namespace AlacrittyConfigUI.Services;

public class ThemeService
{
    private static readonly ILogger Logger = Log.ForContext<ThemeService>();
    private const string BuiltInPrefix = "AlacrittyConfigUI.Assets.BuiltInThemes.";
    ...
    internal static string GetAppDataDirectory()
    {
        ...
        if (!string.IsNullOrEmpty(xdgData))
            return Path.Combine(xdgData, "AlacrittyConfigUI");

        var home = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile);
        return Path.Combine(home, ".local", "share", "AlacrittyConfigUI");
    }
